-- ============================================================
--  EcijaComarca Media Manager — Database Schema
--  Clean install: run this file, then let setup.ps1 / setup.sh
--  insert user accounts with properly hashed passwords.
--  Safe to re-run on an empty database.
-- ============================================================

CREATE DATABASE IF NOT EXISTS ecijacomarca
...
CREATE TABLE IF NOT EXISTS categories (
    ...
    parent_id   INT,
    media_count INT GENERATED ALWAYS AS (0) VIRTUAL,   -- updated via trigger or app
    created_at  TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE IF NOT EXISTS media_items (
    ...
    width                INT,          -- pixels  (video/image)
    height               INT,          -- pixels  (video/image)
    thumbnail_path       TEXT,
    -- Content metadata
...
-- NOTE: user accounts are created by the setup script (setup.ps1 / setup.sh)
-- so that passwords are properly bcrypt-hashed at install time.
-- Default storage location is also configured by the setup script.
