...
LOAD DATA INFILE '/bd.csv' INTO TABLE import_csv FIELDS TERMINATED BY ',' ENCLOSED BY '"' LINES TERMINATED BY '\n' IGNORE 1 ROWS (
    `Key`,
    `Item Type`,
    `Publication Year`,
    `Author`,
...
SELECT DISTINCT
    TRIM(
        SUBSTRING_INDEX (SUBSTRING_INDEX (i.Author, ';', n.n), ';', -1)
    )
FROM
    import_csv i
    JOIN numbers n ON CHAR_LENGTH(i.Author) - CHAR_LENGTH(REPLACE (i.Author, ';', '')) >= n.n - 1
WHERE
    i.Author IS NOT NULL
    AND i.Author != '';
...
INSERT INTO
    media_metadata (media_id, meta_key, meta_value)
SELECT
    m.id,
    'Archive',
    i.Archive
